...
truth = abs(squeeze(im(:,:,32,:)));
load(filename);
% recon = unwrap(angle(im),[],3);
recon = abs(im);
...
for i = 1:nE
    ...
    img(513:768,((i-1)*256 + 1):256*i,:) = rgb_image(rot90(err(:,:,i)),rot90(mask),cmap_err,[0,0,0],[]);
end
...
err = err(ind);
custom_colorbar(min(err(:)),max(err(:)),0,cmap_err,12);
end